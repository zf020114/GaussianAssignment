...
class TTFMITHead(BaseDenseHead, BBoxTestMixin):
    ...
    def loss_calc(self,
        ...
        elif self.loss_cls['type']=='ct_focal_loss_mask':
            pos_maps =pos_maps[:,None,:,:].repeat(1,self.num_classes,1,1)
            hm_loss = ct_focal_loss_mask(pred_hm, heatmap, pos_maps) * self.loss_cls['loss_weight']
        else:
            print('error')
        ...
        # wh_loss2 = self.loss_bbox(pred_boxes.reshape(-1,4), boxes.reshape(-1,4),weight = mask.reshape(-1,1).repeat(1,4),avg_factor = avg_factor)
    ...
    def target_single_image(self, gt_boxes, gt_labels, feat_shape):
        ...
        for k in range(boxes_ind.shape[0]):
            ...
            labels[box_target_inds] = box_target_inds.float()[box_target_inds]*(cls_id+1)
