...
class SegDetHead(BaseDenseHead, BBoxTestMixin):
    ...
    def __init__(self,
        ...
        self.conv_seg = Sequential(
            nn.Conv2d(self.channels, self.channels // 4, 3, bias=with_bias, padding=1),
            nn.BatchNorm2d(self.channels // 4), nn.ReLU(inplace=True))
    ...
    def get_bboxes(self,
        ...
        for img_id in range(len(img_metas)):
            ...
            if self.use_sigmoid:
                scores = cls_score.permute(1, 2, 0).reshape(-1, self.num_fg).sigmoid()
            else:
