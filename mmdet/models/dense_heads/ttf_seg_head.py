...
from mmdet.models.losses.boundry_loss import class2one_hot, one_hot2dist, SurfaceLoss,GeneralizedDice
...
class TTFSegHead(BaseDenseHead, BBoxTestMixin):
    ...
    def __init__(self,
        ...
        self.Loss_gdl =GeneralizedDice(idc=np.arange(self.num_classes))
        self.GeneralizedDiceLoss = monai.losses.GeneralizedDiceLoss(softmax=True,to_onehot_y=True)
    ...
    def _build_predictor(self):
        ...
        conv_cls = nn.Conv2d(self.feat_channels, self.num_fg+1, 1)
    ...
    def build_head(self, out_channel, conv_num=1, head_conv_plane=None):
        ...
        inp = self.channels if conv_num <= 0 else head_conv_plane
    ...
    def get_bboxes(self,
        ...
        for img_id in range(len(img_metas)):
            ...
            scores = cls_score.permute(1, 2, 0).reshape(-1, self.num_fg+1)
            ...
            nms_pre = self.test_cfg.nms_pre#cfg.get('nms_pre', -1)
    ...
    def loss_calc(self,
        ...
        hm_loss =self.GeneralizedDiceLoss(pred_hm,labels[:,None,:,:])
        ...
        loss_dict = dict(  loss_heatmap=hm_loss, loss_bbox=wh_loss,acc_seg=acc_seg)
        ...
        if self.iou_branch:
            ...
            flatten_pred_ious = flatten_pred_ious[pos_inds].sigmoid()
        
            target_ious= bbox_overlaps(  flatten_pred_boxes, flatten_target_boxes.detach(), is_aligned=True)
            # iou_loss = torch.abs(flatten_pred_ious - target_ious).mean()
