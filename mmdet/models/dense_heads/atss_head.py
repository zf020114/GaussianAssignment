...
class ATSSHead(AnchorHead):
    ...
    def _get_target_single_ga(self,
        ...
        feat_shape = (img_meta['pad_shape'][0] // (self.anchor_generator.strides[0][0]),
                          img_meta['pad_shape'][1] // (self.anchor_generator.strides[0][0]))
        ...
        bbox_targets = gt_bboxes.new_ones((4, output_h, output_w)) * -1
        ...
        feat_gt_boxes = gt_bboxes / (self.anchor_generator.strides[0][0])
        ...
        feat_hs =torch.clamp(feat_hs, min=0, max=self.max_radius/(self.anchor_generator.strides[0][0]))
        feat_ws =torch.clamp(feat_ws, min=0, max=self.max_radius/(self.anchor_generator.strides[0][0]))
        # we calc the center and ignore area based on the gt-boxes of the origin scale
        # no peak will fall between pixels
        ct_ints = (torch.stack([(gt_bboxes[:, 0] + gt_bboxes[:, 2]) / 2,
                                (gt_bboxes[:, 1] + gt_bboxes[:, 3]) / 2],
                               dim=1) / self.anchor_generator.strides[0][0]).to(torch.int)

        h_radiuses_alpha = (feat_hs / 2. ).int()
        w_radiuses_alpha = (feat_ws / 2. ).int()
